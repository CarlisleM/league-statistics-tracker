...
function hide_faq_page() {
    document.getElementsByClassName('faq_page')[0].style.display = 'none';
}

function show_faq_page() {
    document.getElementsByClassName('faq_page')[0].style.display = 'block';
}

function hide_bethistory_page() {
    document.getElementsByClassName('bets_page')[0].style.display = 'none';
}

function show_bethistory_page() {
    document.getElementsByClassName('bets_page')[0].style.display = 'block';
}
...
function create_faq_page() {
    hide_all();
    show_faq_page();
    let faq_section = document.getElementsByClassName('faq_page')[0];
    faq_section.innerHTML =  "<br><h1>Frequently Asked Questions</h1><br><p>What websites do you use to bet on?</p><br><p>betonline.ag / Bet365</p>";
}

function create_bets_page() {
    hide_all();
    show_bets_page();
    let bets_section = document.getElementsByClassName('bets_page')[0];
    bets_section.innerHTML =  "";
}
...
function change_team_options(leaguename) {
    ...
    } else if (leaguename == 'LCO') {
        var current_league = 4;
        document.getElementById("league").innerHTML = "LCO";
        var teams = {
            lgc: 'Legacy',
            grv: 'Gravitas',
            mmm: 'MAMMOTH',
    ...
    //         'vitb' : 'Vitality.Bee'
    //     };
    } else if (leaguename == 'LVP_SuperLiga') {
        var current_league = 6;
        document.getElementById("league").innerHTML = "LVP_SuperLiga";
        var teams = {
    ...
    fetchUpcomingGames('my_data_dump.json', function(data) {
        for(j in data.upcoming_matches) {
            if (data.upcoming_matches[k].league_id == current_league) {
                ...
                upcoming_match_date.innerHTML = '<p>' + data.upcoming_matches[k].match_day + ' ' + date + ' '  + data.upcoming_matches[k].match_time + ' PST</p>'
...
function change_team_one_select(selected_team) {
    ...
    if (document.getElementById("league").innerHTML == "LCS") {
        ...
        var league = 3;
    } else if (document.getElementById("league").innerHTML == "LCO") { 
        var league = 4;
            ...
            if (((league == data.matches[k].league_id) && (data.matches[k].blue_team == selected_team || data.matches[k].red_team == selected_team)) || ((league == 0) && (data.matches[k].blue_team == selected_team || data.matches[k].red_team == selected_team))) {
                ...
                for (i = 0; i < match_info.length; i++) {
                    ...
                    if (match_info[i] == selected_team) {
                        ...
                        table_two_body += '<td style="background-color:grey;color:black;">';
                        table_two_body += '-';
                    } else {
